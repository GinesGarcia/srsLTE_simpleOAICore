...
bool gtpu::init(std::string gtp_bind_addr_, std::string sgw_addr_, srsenb::pdcp_interface_gtpu* pdcp_, srslte::log* gtpu_log_)
{
  pdcp          = pdcp_;
  gtpu_log      = gtpu_log_;
  ...
  if(0 != srslte_netsource_init(&src, gtp_bind_addr.c_str(), GTPU_PORT, SRSLTE_NETSOURCE_UDP)) {
    printf("PROBLEMA");
    gtpu_log->error("Failed to create source socket on %s:%d\n", gtp_bind_addr.c_str(), GTPU_PORT);
    return false;
  }
  if(0 != srslte_netsink_init(&snk, sgw_addr.c_str(), GTPU_PORT, SRSLTE_NETSINK_UDP)) {
    printf("problemita");
    gtpu_log->error("Failed to create sink socket on %s:%d\n", sgw_addr.c_str(), GTPU_PORT);
    return false;
  }
 
  printf("  --  GTPU  --  %s %s\n", gtp_bind_addr.c_str(), sgw_addr.c_str());
