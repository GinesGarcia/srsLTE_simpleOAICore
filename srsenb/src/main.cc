...
void parse_args(all_args_t *args, int argc, char* argv[]) {
  ...
  common.add_options()
    ...
    ("enb.mme_addr",      bpo::value<string>(&args->enb.s1ap.mme_addr)->default_value("127.0.0.1"),"MME addr")
    ("enb.s1c_bind_addr", bpo::value<string>(&args->enb.s1ap.s1c_bind_addr),		           "eNB IP addr for S1-C")
    ("enb.sgw_addr",      bpo::value<string>(&args->enb.s1ap.sgw_addr),		                   "S-GW IP addr")
    ("enb.gtp_bind_addr", bpo::value<string>(&args->enb.s1ap.gtp_bind_addr),                       "eNB IP addr for S1-U")
